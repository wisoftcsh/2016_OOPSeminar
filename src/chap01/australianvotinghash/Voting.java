...
public class Voting {
    ...
    public void read() {
        ...
        while (loop != informationArrayList.size()) {
            if (informationArrayList.get(loop).isEmpty() &&
                ...
                boxArray = Box.votingBox(target); //투표함
                voter = boxArray.size();
                voting();
    ...
    public void candidateCreate() {
        ...
        for (int i = 1; i <= candidateNumber; i++) {
            ...
            candidateList.put(i, candidate);
        }
    }

    public void zeroRemove() { // 득표수가 0인 후보자 탈락
        for (int i = 1; i <= candidateNumber; i++) {
            if (candidateList.get(i).NumberCounter == 0) {
    ...
    public int minDetermine() {
        ...
        for (int i = 1; i <= candidateNumber; i++) {
            if (candidateList.get(i) != null) {
                if (min > candidateList.get(i).getNumberCounter()) {
                    min = candidateList.get(i).getNumberCounter();
                }
            } else if (candidateList.get(i) == null) {
                continue;
            }
        }
        return min;
    ...
    public void removeHash(int min) {
        for (int i = 1; i <= candidateNumber; i++) {
            if (candidateList.get(i) != null) {
                if (candidateList.get(i).getNumberCounter() == min) {
                    candidateList.remove(i);
                    boxRemove(i);
                }
            } else {
    ...
    public int isMajority() {
        ...
        for (int i = 1; i <= candidateNumber; i++) {
            if (candidateList.get(i) != null) {
                if (max < candidateList.get(i).getNumberCounter()) {
                    max = candidateList.get(i).getNumberCounter();
                }
            } else {
                continue;
            }
        ...
        if (majority >= 50) {
            for (int i = 1; i <= candidateNumber; i++) {
                if (candidateList.get(i) != null) {
                    if (candidateList.get(i).getNumberCounter() == max) {
                        System.out.println(candidateList.get(i).getName());  //output
                        textInOut.ouput(candidateList.get(i).getName());
                    }
                }
            }
            return 1;
    ...
    public void counterClear(){
        for(int i = 1; i<=candidateNumber;i++){
            if(candidateList.get(i) != null){
                candidateList.get(i).NumberCounter = 0;
            }else{
                continue;
            }
        }
        ...
        while (true) { // 1순위 투표 집계
            ...
            for (int i = 0; i < boxArray.size(); i++) {
                if (boxArray.get(i).get(0).equals(num)) {
                    candidateList.get(num).setNumberCounter();
                }
            }
            num++;
        }
